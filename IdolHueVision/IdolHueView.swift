    ...
    private func layout(geometry: GeometryProxy3D) async {
        ...
        if idolEntitiesRoot.children.count != idols.count {
            ...
            if #available(visionOS 27, *) {
                ...
                let pbr = try! graph.addNode(library.makeNode(from: library.definition(named: "ND_realitykit_pbr_surfaceshader")!))
                let iid = try! graph.addNode(library.makeNode(from: library.definition(named:"ND_realitykit_instance_id")!))
                let itof = try! graph.addNode(library.makeNode(from: library.definition(named:"ND_convert_integer_float")!))
                let color = try! graph.addNode(library.makeNode(from: library.definition(named:"ND_combine3_color3")!))
                let rdiv = try! graph.addNode(library.makeNode(from: library.definition(named:"ND_divide_float")!))
                try! graph.connect(iid, to: itof, inputPort: "in")
                try! graph.connect(itof, to: rdiv, inputPort: "in1")
                try! graph.connect(graph.addConstant(.float(Float(idols.count))), to: rdiv, inputPort: "in2")
                try! graph.connect(rdiv, to: color, inputPort: "in1")
                try! graph.connect(color, to: pbr, inputPort: "baseColor")
                try! graph.connect(pbr, to: graph.results.name, inputPort: "out")
